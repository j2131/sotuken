...
class Vector3 {
   public:
    ...
    constexpr Vector3(const Vector3 &src) : x(src.x), y(src.y), z(src.z) {}
    ...
    T operator*(const Vector3 &rhs) const { return (x * rhs.x) + (y * rhs.y) + (z * rhs.z); }

    Vector3 operator^(const Vector3 &rhs) const { return Vector3((y * rhs.z - z * rhs.y), (z * rhs.x - x * rhs.z), (x * rhs.y - y * rhs.x)); }
