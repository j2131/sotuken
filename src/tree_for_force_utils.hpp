...
namespace ParticleSimulator {
    ...
    // AllGatherParticle is used in vdwtest
    ...
    // liner search
    template<class Ttp, typename Tmorton_key>
...
// for PMMM
template <class Ttp, typename Tmorton_key>
inline S32 GetPartitionID(const Ttp tp[], const S32 left, const S32 right, const S32 ref, const S32 lev, const Tmorton_key &morton_key) {
...
template <class Ttp, typename Tmorton_key, Tmorton_key, int levtype = 0>
inline S32 GetPartitionIDsub(const Ttp tp[], const S32 left, const S32 right, const S32 ref, const S32 lev, const Tmorton_key &morton_key) {
...
inline S32 GetPartitionIDnew(const Ttp tp[],  // slightly faster on X86 but not on Fugaku...
                             const S32 left, const S32 right, const S32 ref, const S32 lev, const Tmorton_key &morton_key) {
    if (lev <= morton_key.keylevelmaxhigh()) {
        ...
        auto levarg = (morton_key.keylevelmaxlow() - (lev - morton_key.keylevelmaxhigh())) * 3;
...
inline void SearchSendParticleLongScatter(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Tep> &ep_first,
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        //                << i);
        open_bits |=
            (((pos_target_box.getDistanceMinSQ(pos_tmp) <= r_crit_sq) || (pos_target_box.contained(tc_first[adr_tc + i].geo_.getVertexOut()))) << i);
    }
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                SearchSendParticleLongScatter<Ttc, Tep>(tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, id_ep_send, id_sp_send, pos_target_box,
...
template <class Ttc, class Tep>
inline void SearchSendParticleLongSymmetry(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Tep> &ep_first,
                                           ReallocatableArray<S32> &id_ep_send, ReallocatableArray<S32> &id_sp_send, const F64ort &pos_target_box_in,
                                           const F64ort &pos_target_box_out, const F64 r_crit_sq, const S32 n_leaf_limit) {
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
        const F64vec pos_tmp = tc_first[adr_tc + i].mom_.getPos();
        /*
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                SearchSendParticleLongSymmetry<Ttc, Tep>(tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, id_ep_send, id_sp_send,
...
inline F64ort makeChildCellBox(const S32 i, const F64ort &cell_box) {
    ...
    for (S32 k = 0; k < DIMENSION; k++) {
        child_cell_high[k] = cell_box.high_[k] - (((i >> (DIMENSION - 1 - k)) & 0x1) ^ 0x1) * half_length;
        child_cell_low[k] = cell_box.low_[k] + ((i >> (DIMENSION - 1 - k)) & 0x1) * half_length;
    }
...
inline void SearchSendParticleLongCutoff(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Tep> &ep_first,
                                         ReallocatableArray<Tep> &ep_send, ReallocatableArray<Tsp> &sp_send, const F64ort &cell_box,
                                         const F64ort &pos_target_domain, const F64 r_crit_sq, const F64 r_cut_sq, const S32 n_leaf_limit,
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                const F64ort child_cell_box = makeChildCellBox(i, cell_box);
                SearchSendParticleLongCutoff<Ttc, Tep, Tsp>(tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_send, sp_send, child_cell_box,
                                                            pos_target_domain, r_crit_sq * 0.25, r_cut_sq, n_leaf_limit, shift);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
...
template <class Ttc, class Tep, class Tsp>
inline void SearchSendParticleLongCutoffWithRootCellCheck(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
                                                          const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_send,
                                                          ReallocatableArray<Tsp> &sp_send, const F64ort &cell_box, const F64ort &pos_target_domain,
                                                          const F64 r_crit_sq, const F64 r_cut_sq, const S32 n_leaf_limit, const F64ort pos_root_cell,
                                                          const F64vec &shift = 0.) {
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
        F64vec pos_tmp = tc_first[adr_tc + i].mom_.getPos();
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                const F64ort child_cell_box = makeChildCellBox(i, cell_box);
                SearchSendParticleLongCutoffWithRootCellCheck<Ttc, Tep, Tsp>(tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_send, sp_send,
                                                                             child_cell_box, pos_target_domain, r_crit_sq * 0.25, r_cut_sq,
                                                                             n_leaf_limit, pos_root_cell, shift);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                // ep_send.reserve( ep_send.size()+n_child );
                ep_send.reserveEmptyAreaAtLeast(n_child);
...
template <class Ttc, class Tep>
inline void SearchSendParticleLongCutoffForZeroTheta(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc,
                                                     const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_send,
                                                     const F64ort &cell_box, const F64ort &pos_target_domain, const F64 r_cut_sq,
                                                     const S32 n_leaf_limit, const F64ort pos_root_cell, const F64vec &shift = 0.) {
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
        const F64ort child_cell_box = makeChildCellBox(i, cell_box);
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                const F64ort child_cell_box = makeChildCellBox(i, cell_box);
                SearchSendParticleLongCutoffForZeroTheta<Ttc, Tep>(tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_send, child_cell_box,
                                                                   pos_target_domain, r_cut_sq, n_leaf_limit, pos_root_cell, shift);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                ep_send.reserveEmptyAreaAtLeast(n_child);
                for (S32 ip = 0; ip < n_child; ip++) {
...
void MakeInteractionListLongEPSP(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Ttp> &tp_first,
                                 const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list, const ReallocatableArray<Tsp> &sp_first,
                                 ReallocatableArray<Tsp> &sp_list, const F64ort &pos_target_box, const F64 r_crit_sq, const S32 n_leaf_limit) {
    U32 open_bits = 0;
// #ifdef __HPC_ACE__
#ifdef FAST_WALK_K
    ...
    for (S32 i = 0; i < N_CHILDREN / 2; i++) {
        ...
        _fjsp_v2r8 dr_sq_tmp = _fjsp_madd_v2r8(dz, dz, _fjsp_madd_v2r8(dy, dy, _fjsp_mul_v2r8(dx, dx)));
    ...
    __m256 jx = _mm256_set_m128_forgcc(_mm_loadu_ps(&mom4.x), _mm_loadu_ps(&mom0.x));
    __m256 jy = _mm256_set_m128_forgcc(_mm_loadu_ps(&mom5.x), _mm_loadu_ps(&mom1.x));
    __m256 jz = _mm256_set_m128_forgcc(_mm_loadu_ps(&mom6.x), _mm_loadu_ps(&mom2.x));
    __m256 jw = _mm256_set_m128_forgcc(_mm_loadu_ps(&mom7.x), _mm_loadu_ps(&mom3.x));
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0) {
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                ...
                MakeInteractionListLongEPSP<Ttc, Ttp, Tep, Tsp>(tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, ep_list, sp_first,
                                                                sp_list, pos_target_box, r_crit_sq, n_leaf_limit);
#else
                MakeInteractionListLongEPSP<Ttc, Ttp, Tep, Tsp>(tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, ep_list, sp_first,
                                                                sp_list, pos_target_box, r_crit_sq * 0.25, n_leaf_limit);
#endif
            } else {
...
template <class Ttc, class Ttp, class Tep, class Tsp>
void MakeInteractionListLongScatterEPSP(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Ttp> &tp_first,
                                        const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
                                        const ReallocatableArray<Tsp> &sp_first, ReallocatableArray<Tsp> &sp_list, const F64ort &pos_target_box,
                                        const F64 r_crit_sq, const S32 n_leaf_limit) {
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
        const F64vec pos_tmp = tc_first[adr_tc + i].mom_.getPos();
        ...
        // open_bits |= ( (pos_target_box.getDistanceMinSQ(pos_tmp) <= r_crit_sq) << i);
        open_bits |= (((pos_target_box.getDistanceMinSQ(pos_tmp) <= r_crit_sq) || (pos_target_box.contained(box_tmp))) << i);
    }
    for (S32 i = 0; i < N_CHILDREN; i++) {
        const S32 adr_tc_child = adr_tc + i;
        const Ttc *tc_child = tc_first.getPointer(adr_tc_child);
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                // #ifdef DEBUG_1023_2
#ifdef PARTICLE_SIMULATOR_INTERACTION_LIST_ALL
                MakeInteractionListLongScatterEPSP<Ttc, Ttp, Tep, Tsp>(tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, ep_list,
                                                                       sp_first, sp_list, pos_target_box, r_crit_sq, n_leaf_limit);
#else
                MakeInteractionListLongScatterEPSP<Ttc, Ttp, Tep, Tsp>(tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, ep_list,
                                                                       sp_first, sp_list, pos_target_box, r_crit_sq * 0.25, n_leaf_limit);
#endif
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                ep_list.reserveEmptyAreaAtLeast(n_child);
...
template <class Ttc, class Ttp, class Tep, class Tsp>
void MakeInteractionListLongSymmetryEPSP(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Ttp> &tp_first,
                                         const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
                                         const ReallocatableArray<Tsp> &sp_first, ReallocatableArray<Tsp> &sp_list, const F64ort &pos_target_box_in,
                                         const F64ort &pos_target_box_out, const F64 r_crit_sq, const S32 n_leaf_limit) {
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
        const F64vec pos_tmp = tc_first[adr_tc + i].mom_.getPos();
        ...
        open_bits |= (((pos_target_box_in.getDistanceMinSQ(pos_tmp) <= r_crit_sq) || (pos_target_box_out.contained(box_tmp_in)) ||
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                MakeInteractionListLongSymmetryEPSP<Ttc, Ttp, Tep, Tsp>(tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, ep_list,
                                                                        sp_first, sp_list, pos_target_box_in, pos_target_box_out, r_crit_sq * 0.25,
                                                                        n_leaf_limit);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                ep_list.reserveEmptyAreaAtLeast(n_child);
...
template <class Ttc, class Ttp, class Tep, class Tsp>
void MakeInteractionListLongCutoffEPSP(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Ttp> &tp_first,
                                       const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
                                       const ReallocatableArray<Tsp> &sp_first, ReallocatableArray<Tsp> &sp_list, const F64ort &cell_box,
                                       const F64ort &pos_target_box, const F64 r_crit_sq, const F64 r_cut_sq, const S32 n_leaf_limit) {
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
        const F64vec pos_tmp = tc_first[adr_tc + i].mom_.getPos();
        open_bits |= ((pos_target_box.getDistanceMinSQ(pos_tmp) <= r_crit_sq) << i);
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                const F64ort child_cell_box = makeChildCellBox(i, cell_box);
                MakeInteractionListLongCutoffEPSP<Ttc, Ttp, Tep, Tsp>(tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, ep_list, sp_first,
                                                                      sp_list, child_cell_box, pos_target_box, r_crit_sq * 0.25, r_cut_sq,
                                                                      n_leaf_limit);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
...
void MakeInteractionListLongCutoffEPForZeroTheta(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Ttp> &tp_first,
                                                 const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list, const F64ort &cell_box,
                                                 const F64ort &pos_target_box, const F64 r_cut_sq, const S32 n_leaf_limit) {
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
#if 0
            const F64ort child_cell_box = makeChildCellBox(i, cell_box);
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if (n_child == 0)
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                const F64ort child_cell_box = makeChildCellBox(i, cell_box);
                MakeInteractionListLongCutoffEPForZeroTheta<Ttc, Ttp, Tep>(tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, ep_list,
                                                                           child_cell_box, pos_target_box, r_cut_sq, n_leaf_limit);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                ep_list.reserveEmptyAreaAtLeast(n_child);
                for (S32 ip = 0; ip < n_child; ip++) {
...
inline void MakeListUsingOuterBoundary(const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first, ReallocatableArray<Tep3> &ep_list,
                                       const F64ort &pos_target_box,  // position of domain
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        if ((open_bits >> i) & 0x1) {
            ...
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                MakeListUsingOuterBoundary<Ttc, Tep2, Tep3>(tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_list, pos_target_box, n_leaf_limit,
                                                            shift);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                ep_list.reserveEmptyAreaAtLeast(n_child);
...
template <class Ttc, class Tep2, class Tep3>
inline void MakeListUsingOuterBoundaryWithRootCellCheck(const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first,
                                                        ReallocatableArray<Tep3> &ep_list,
                                                        const F64ort &pos_target_box,  // position of domain
                                                        const S32 n_leaf_limit, const F64ort pos_root_cell, const F64vec &shift = F64vec(0.0)) {
    U32 open_bits = 0;
    for (S32 i = 0; i < N_CHILDREN; i++) {
        // open_bits |= (pos_target_box.contained( tc_first[adr_tc+i].mom_.getVertexOut() ) << i);
        open_bits |= (pos_target_box.contained(tc_first[adr_tc + i].geo_.getVertexOut()) << i);
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        if ((open_bits >> i) & 0x1) {
            ...
            if (n_child == 0) continue;
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                MakeListUsingOuterBoundaryWithRootCellCheck<Ttc, Tep2, Tep3>(tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_list,
...
template <class Ttc, class Tep2, class Tep3>
inline void MakeListUsingOuterBoundaryAndInnerBoundary(const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first, ReallocatableArray<Tep3> &ep_list,
    ...
    for (S32 i = 0; i < N_CHILDREN / 4; i++) {
        ...
        const _fjsp_v2r8 mask_x_io_ji_a =
            _fjsp_and_v2r8(_fjsp_cmple_v2r8(xi_l_out, xj_h_in_a), _fjsp_cmple_v2r8(xj_l_in_a, xi_h_out));  // i:out, j:in
        ...
        const _fjsp_v2r8 mask_x_ii_jo_a =
            _fjsp_and_v2r8(_fjsp_cmple_v2r8(xi_l_in, xj_h_out_a), _fjsp_cmple_v2r8(xj_l_out_a, xi_h_in));  // i:in, j:out
        ...
        const _fjsp_v2r8 mask_y_io_ji_a =
            _fjsp_and_v2r8(_fjsp_cmple_v2r8(yi_l_out, yj_h_in_a), _fjsp_cmple_v2r8(yj_l_in_a, yi_h_out));  // i:out, j:in
        ...
        const _fjsp_v2r8 mask_y_ii_jo_a =
            _fjsp_and_v2r8(_fjsp_cmple_v2r8(yi_l_in, yj_h_out_a), _fjsp_cmple_v2r8(yj_l_out_a, yi_h_in));  // i:in, j:out
        ...
        const _fjsp_v2r8 mask_z_io_ji_a =
            _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l_out, zj_h_in_a), _fjsp_cmple_v2r8(zj_l_in_a, zi_h_out));  // i:out, j:in
        ...
        const _fjsp_v2r8 mask_z_ii_jo_a =
            _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l_in, zj_h_out_a), _fjsp_cmple_v2r8(zj_l_out_a, zi_h_in));  // i:in, j:out
        const _fjsp_v2r8 mask_z_ii_jo_b = _fjsp_and_v2r8(_fjsp_cmple_v2r8(zi_l_in, zj_h_out_b), _fjsp_cmple_v2r8(zj_l_out_b, zi_h_in));

        const _fjsp_v2r8 mask_tmp_a = _fjsp_and_v2r8(_fjsp_or_v2r8(_fjsp_and_v2r8(mask_x_io_ji_a, _fjsp_and_v2r8(mask_y_io_ji_a, mask_z_io_ji_a)),
                                                                   _fjsp_and_v2r8(mask_x_ii_jo_a, _fjsp_and_v2r8(mask_y_ii_jo_a, mask_z_ii_jo_a))),
                                                     ONE);

        const _fjsp_v2r8 mask_tmp_b = _fjsp_and_v2r8(_fjsp_or_v2r8(_fjsp_and_v2r8(mask_x_io_ji_b, _fjsp_and_v2r8(mask_y_io_ji_b, mask_z_io_ji_b)),
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if ((open_bits >> i) & 0x1) {
            ...
            if (!(tc_child[i].isLeaf(n_leaf_limit))) {
                MakeListUsingOuterBoundaryAndInnerBoundary<Ttc, Tep2, Tep3>(tc_first, tc_child[i].adr_tc_, ep_first, ep_list, pos_target_box_out,
                                                                            pos_target_box_in, n_leaf_limit, shift);
            } else {
...
template <class Ttc, class Tep2, class Tep3>
inline void MakeListUsingInnerBoundary(const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first, ReallocatableArray<Tep3> &ep_list,
                                       const F64ort &pos_target_box,  // position of domain
                                       const S32 n_leaf_limit, const F64vec &shift = F64vec(0.0)) {
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if ((open_bits >> i) & 0x1) {
            ...
            if (n_child == 0) continue;
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                MakeListUsingInnerBoundary<Ttc, Tep2, Tep3>(tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_list, pos_target_box, n_leaf_limit,
                                                            shift);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                // ep_list.reserve( ep_list.size()+n_child );
...
template <class Ttc>
inline void IsOverlappedUsingOuterBoundaryOfTreeImpl(const Ttc *tc_first, const S32 adr_tc, const F64ort &pos_box, const S32 n_leaf_limit,
                                                     U32 &is_overlapped) {
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        ...
        if ((open_bits >> i) & 0x1) {
            if (!(tc_child->isLeaf(n_leaf_limit))) {
                IsOverlappedUsingOuterBoundaryOfTreeImpl(tc_first, tc_first[adr_tc_child].adr_tc_, pos_box, n_leaf_limit, is_overlapped);
            } else {
                // leaf and overlapped
                is_overlapped = 1;
                return;
...
template <class Ttc>
inline U32 IsOverlappedUsingOuterBoundaryOfTree(const Ttc *tc_first, const F64ort &pos_box, const S32 n_leaf_limit) {
    U32 is_overlapped = 0;
...
inline void MakeLETListByDoubleWalkImpl(const Ttca *tc_first_A, const Ttcb *tc_first_B, const S32 adr_tc_B, const Tepb *ep_first_B,
                                        const F64ort &pos_domain, const S32 n_leaf_limit_A, const S32 n_leaf_limit_B,
    ...
    for (S32 i = 0; i < N_CHILDREN; i++) {
        if ((open_bits >> i) & 0x1) {
            ...
            if (n_child == 0)
                ...
                MakeLETListByDoubleWalkImpl(tc_first_A, tc_first_B, tc_first_B[adr_tc_child].adr_tc_, ep_first_B, pos_domain, n_leaf_limit_A,
                                            n_leaf_limit_B, id_ptcl_send);
            } else {
                S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                for (S32 ip = 0; ip < n_child; ip++, adr_ptcl_tmp++) {
#if defined(ORIGINAL_SCATTER_MODE)
...
template <class Ttca, class Ttcb, class Tepb>
inline void MakeLETListByDoubleWalk(const Ttca *tc_first_A, const Ttcb *tc_first_B, const Tepb *ep_first_B, const F64ort &pos_domain,
                                    const S32 n_leaf_limit_A, const S32 n_leaf_limit_B, ReallocatableArray<S32> &id_ptcl_send) {
    const S32 adr_tc_B = N_CHILDREN;
    if (!tc_first_B->isLeaf(n_leaf_limit_B)) {
        MakeLETListByDoubleWalkImpl(tc_first_A, tc_first_B, adr_tc_B, ep_first_B, pos_domain, n_leaf_limit_A, n_leaf_limit_B, id_ptcl_send);
    } else {
        // if( tc_first_B->mom_.getVertexOut().contained(pos_domain) || IsOverlappedUsingOuterBoundaryOfTree(tc_first_A,
        // tc_first_B->mom_.getVertexIn(), n_leaf_limit_A)){
        if (tc_first_B->geo_.getVertexOut().contained(pos_domain) ||
            IsOverlappedUsingOuterBoundaryOfTree(tc_first_A, tc_first_B->geo_.getVertexIn(), n_leaf_limit_A)) {
...
inline void SearchNeighborListOneParticleScatter(const Tvec &pos_target, const Ttc *tc_first, const Ttp *tp_first, const S32 adr_tc,
                                                 const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list, const S32 n_leaf_limit) {
#if 1
    ...
    if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
        ...
        for (S32 i = 0; i < N_CHILDREN; i++) {
            ...
            if (n_child == 0)
                ...
                SearchNeighborListOneParticleScatter(pos_target, tc_first, tp_first, adr_tc_child + i, ep_first, ep_list, n_leaf_limit);
            }
        }
    }
...
template <class Tvec, class Ttc, class Ttp, class Tep>
inline void SearchNeighborListOneParticleGather(const Tvec &pos_target, const F64 &r_search_sq, const Ttc *tc_first, const Ttp *tp_first,
                                                const S32 adr_tc, const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
    ...
    if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
        ...
        for (S32 i = 0; i < N_CHILDREN; i++) {
            ...
            if (n_child == 0)
                ...
                SearchNeighborListOneParticleGather(pos_target, r_search_sq, tc_first, tp_first, adr_tc_child + i, ep_first, ep_list, n_leaf_limit);
            }
        }
    }
}
...
template <class Tvec, class Ttc, class Ttp, class Tep>
inline void SearchNeighborListOneParticleSymmetry(const Tvec &pos_target, const F64 &r_search_i_sq, const Ttc *tc_first, const Ttp *tp_first,
                                                  const S32 adr_tc, const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
                                                  const S32 n_leaf_limit) {
#if 1
    if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
    ...
    if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
        ...
        for (S32 i = 0; i < N_CHILDREN; i++) {
            ...
            if (n_child == 0)
                ...
                SearchNeighborListOneParticleSymmetry(pos_target, r_search_i_sq, tc_first, tp_first, adr_tc_child + i, ep_first, ep_list,
                                                      n_leaf_limit);
            }
        }
    }
...
template <class Tvec, class Ttc, class Ttp, class Tep>
inline void SearchNeighborListOneParticleNumber(const Tvec &pos_target, const S32 n_ngb_target, const Ttc *tc_first, const Ttp *tp_first,
                                                const S32 adr_tc, const ReallocatableArray<Tep> &ep_first,
                                                std::vector<std::pair<Tep, F64>> &ep_md_list, F64ort &vertex_ep_list, const S32 n_leaf_limit) {
    if (tc_first[adr_tc].isLeaf(n_leaf_limit)) {
        ...
        for (S32 ip = 0; ip < n_tmp; ip++, adr_tp_tmp++) {
            ...
            if (ep_md_list.size() < n_ngb_target) {
                ...
                std::sort(ep_md_list.begin(), ep_md_list.end(),
                          [](const std::pair<Tep, F64> &l, const std::pair<Tep, F64> &r) -> bool { return l.second > r.second; });
            } else {
                if (ep_md_new.second < ep_md_list.front().second) {
                    is_updated = true;
                    if (ep_md_list.size() > 1) {
                        for (S32 i = 1; i < ep_md_list.size(); i++) {
                            if (ep_md_list[i].second > ep_md_new.second)
                                ep_md_list[i - 1] = ep_md_list[i];
                            else
                                ep_md_list[i - 1] = ep_md_new;
                        }
                    } else {
        ...
        if (is_updated) {
            F64ort vertex;
            for (S32 i = 0; i < ep_md_list.size(); i++) vertex.merge(ep_md_list[i].first.getPos());
        ...
        for (S32 i = 0; i < N_CHILDREN; i++) {
            ...
            if (n_child == 0)
                continue;
            else if ((ep_md_list.size() < n_ngb_target) || (ep_md_list.size() == n_ngb_target && vertex_cell.overlapped(vertex_ep_list))) {
                SearchNeighborListOneParticleNumber(pos_target, n_ngb_target, tc_first, tp_first, adr_tc_child + i, ep_first, ep_md_list,
                                                    vertex_ep_list, n_leaf_limit);
            }
        }
    }
}
...
template <class Ttc, class Tep2, class Tep3>
inline CountT MakeListUsingOuterBoundaryAndInnerBoundaryIteration(const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first,
                                                                  ReallocatableArray<Tep3> &ep_list,
                                                                  const F64ort &pos_target_box_out,  // position of domain
                                                                  const F64ort &pos_target_box_in,   // position of domain
                                                                  const S32 n_leaf_limit, const F64vec &shift = F64vec(0.0)) {
...
template <class Ttc, class Tep2, class Tep3>
inline CountT MakeListUsingInnerBoundaryIteration(const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first, ReallocatableArray<Tep3> &ep_list,
                                                  const F64ort &pos_target_box,  // position of domain
                                                  const S32 n_leaf_limit, const F64vec &shift = F64vec(0.0)) {
...
template <class Ttc, class Tep2, class Tep3>
inline void MakeListUsingOuterBoundaryIteration(const Ttc *tc_first, const S32 adr_tc, const Tep2 *ep_first, ReallocatableArray<Tep3> &ep_list,
                                                const F64ort &pos_target_box,  // position of domain
                                                const S32 n_leaf_limit, const F64vec &shift = F64vec(0.0)) {
    CountT n_cell_open = 0;
    // static __thread Stack<S32, 1000> adr_tc_stack;
...
#if 1
template <class Ttc, class Ttp, class Tep, class Tsp>
inline CountT MakeInteractionListLongEPSPIteration(const ReallocatableArray<Ttc> &tc_first, const S32 adr_tc, const ReallocatableArray<Ttp> &tp_first,
                                                   const ReallocatableArray<Tep> &ep_first, ReallocatableArray<Tep> &ep_list,
...
#include "tree_for_force_utils_make_jplist.hpp"
#include "tree_for_force_utils_make_ipg.hpp"
#include "tree_for_force_utils2.hpp"
//#include "tree_for_force_utils_find_scatter_particle.hpp"
#include "tree_for_force_utils_pmmm.hpp"
